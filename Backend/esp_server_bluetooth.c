...
#define IMAGE_NUM 10
#define IMAGE_PATH "/home/ascc/asccbot_v3/asccChatBot/wearableDevice/Images/"

int init_data_size = 20;
...
void func(int sockfd) 
{
    ...
    int anotherImage = 0;
    int timeOutImageFlag = 0;

	while(1){
		
		if (flag == 0){
                    if (timeOutImageFlag == 1){
                        break;
...
//------------------------------------------------------------------------------
    ...
    while(1) {
        ...
        if (rfcommsock == -1) {
            printf("rfcommsock == -1, reconnecting \n");
            continue;
        }
